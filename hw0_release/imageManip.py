...
def mix_images(image1, image2, channel1, channel2):
    ...
    hw = image1.shape[1] // 2
    out = np.concatenate((image1[:, :hw, :], image2[:, hw:, :]), axis=1)
    # END YOUR CODE
...
def mix_quadrants(image):
    ...
    top_left = image[:HH, :HW]
    top_right = image[:HH, HW:]
    bottom_left = image[HH:, :HW]
    bottom_right = image[HH:, HW:]

    left_out = np.concatenate(
        (rgb_exclusion(top_left, "R"), bottom_left ** 0.5), axis=0)
    right_out = np.concatenate(
        (dim_image(top_right), rgb_exclusion(bottom_right, "R")), axis=0)

    out = np.concatenate((left_out, right_out), axis=1)
    # END YOUR CODE
