...
def harris_corners(img, window_size=3, k=0.04):
    ...
    # YOUR CODE HERE
    dx2 = convolve(dx * dx, window, mode="constant", cval=0.0)
    dxdy = convolve(dx * dy, window, mode="constant", cval=0.0)
    dy2 = convolve(dy * dy, window, mode="constant", cval=0.0)
    M = np.array([[dx2, dxdy], [dxdy, dy2]])
    M = np.transpose(M, axes=(2, 3, 0, 1))
    response = np.linalg.det(M) - k * np.trace(M.transpose((2, 3, 0, 1))) ** 2
...
def simple_descriptor(patch):
    ...
    patch_normalized = (patch - np.mean(patch)) / std
    feature = np.ravel(patch_normalized)
    # END YOUR CODE
    return feature
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    # YOUR CODE HERE
    # find out index of the minimum distacne
    min_indexs = np.argmin(dists, axis=1)
    # sort distances
    sorted_dists = np.sort(dists, axis=1)
    # compute the ratio between the minimum distance and the sencond minimum distance
    dists_ratio = sorted_dists[:, 0] / sorted_dists[:, 1]
    # construct a mask array
    mask = dists_ratio < threshold
    # concatenate the index of two discriptors
    matches = np.concatenate(
        (np.arange(N)[mask][:, None], min_indexs[mask][:, None]), axis=1)
    # END YOUR CODE
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for _ in range(n_iters):
        ...
        H = np.linalg.lstsq(matched2[index],
                            matched1[index], rcond=None)[0]
        H[:, 2] = np.array([0, 0, 1])
        residual = np.linalg.norm(matched2 @ H - matched1, ord=2, axis=1)
        mask = residual < threshold
        num_points = np.sum(mask)
        if num_points > n_inliers:
            max_inliers = mask.copy()
            n_inliers = num_points
